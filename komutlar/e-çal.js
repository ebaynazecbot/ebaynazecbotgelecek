...
exports.run = async(client, message, args) => { 
  ...
  if(user.id === message.author.id) return message.reply('Kendini soyamazsın!')
  
  if(user.bot) return message.reply('Botu soyamazsın!')
 ...
 db.set(`puan_${user.id}`, 200)   
  ...
  if(random === "başarısız") {
    ...
    let kesilecek = kontrol / 2
         let embed = new Discord.RichEmbed()
    .setTitle('Görev Başarısız!')
    .setDescription(user +"'i soymaya çalışırken,Yerde taşa takıldın ve halkdan dayak yedikten sonra polise yakalandın.Mahkeme Paranın yarısına, **"+kesilecek+"** el koymaya karar verdi.") 
    .setColor('RED')
    .setThumbnail(message.author.avatarURL) 
    .setFooter(client.user.username, client.user.avatarURL)
    message.channel.send(embed)
